...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    // getting current date (hozirgi sana;)
    let today = new Date()
    for (let i = 0; i < 7; i++) {

      // getting date with index (index bn sanani olish )
      let currentDate = new Date(today);
      currentDate.setDate(today.getDate() + i); // bugundan keyingi kunni ifodalaydi.
    ...
    docInfo && (
      <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div>
            <img
              className="bg-slate-600 w-full sm:maxw-72 rounded-lg"
              src={docInfo.image}
              alt=""
            />
          </div>
          ...
          <div className="flex-1 border border-gray-600 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
            {/* ---------------Doc Info : name, degree, experions --------- */}
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <button className="bg-purple-500 text-white text-sm font-light px-14 py-3 rounded-full my-6">Book an Appointment</button>
        </div>
